...
class SuperTuxDataset(Dataset):
    ...
    def __init__(self, dataset_path):
      # Opening the labels.csv file.
      with open(path.join(dataset_path, 'labels.csv')) as labels_file:
        # Creating the CSV reader for the labels.csv file.
        labels_reader = csv.reader(labels_file)

        # Calling next() to skip the header line.
        next(labels_reader)

        # Creating the dataset from the input values.
        self.images = []
        self.labels = []
        for row in labels_reader:
          self.images.append(path.join(dataset_path, row[0]))
          self.labels.append(LABEL_NAMES.index(row[1])) 

        self.transform = transforms.Compose([
          transforms.ColorJitter(0.5, 0.25, 0.25, 0.25),
    ...
    def __getitem__(self, idx):
      # Returns a tuple for a given index: (image, label)
      return (
        F.to_tensor(self.transform(Image.open(self.images[idx]))), 
        self.labels[idx]
      )
...
def load_dense_data(dataset_path, num_workers=0, batch_size=32, **kwargs):
    ...
    transform = dense_transforms.Compose([
      dense_transforms.ColorJitter(0.5, 0.25, 0.25, 0.25),
